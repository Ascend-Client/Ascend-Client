...
public class AccessorMapper implements MixinMethodMapper {
    @Override
    public void mapMixin(ClassNode n1, MethodNode node, AnnotationNode annotation, Map<String, String> mappings) {
        ...
        if(node.name.startsWith("get")) {
            String nameOfAccessed = node.name.toLowerCase().replace("get", "");
            node.name = "get" + ModRemapperUtility.capitalize(mappings.getOrDefault(nameOfAccessed, nameOfAccessed));
        } else {
    ...
    public boolean detect(AnnotationNode annotation) {
        return annotation.desc.equals("Lorg/spongepowered/asm/mixin/gen/Accessor;");
    }
}
