...
public class MixinGameRenderer {
    ...
    public void wefwefewfwef(RenderTickCounter tickCounter, boolean tick, CallbackInfo ci) {
        ...
        if (MotionBlur.get().toggled) {
            ...
            if(blur.shader != null)
                ((PostEffectProcessor) blur.shader).render(tickCounter.getTickDelta(true));
        }

        if(SuperSecretSettings.get().toggled) {
            SuperSecretSettings.get().onUpdate();
            if(SuperSecretSettings.get().shader != null)
                ((PostEffectProcessor) SuperSecretSettings.get().shader).render(tickCounter.getTickDelta(true));
        }
